...
class TabsHelperTest < ActionView::TestCase
  ...
  context 'Tab' do
    should 'return all tabs in order' do
      assert_equal 5, tabs.count
      assert_equal ['edit', 'tagging', 'metadata', 'history', 'admin'], tabs.map {|t| t.name}
    end

    should 'return tabs with expected titles' do
      assert_equal ['Edit', 'Tagging', 'Metadata', 'History and notes', 'Admin'], tabs.map {|t| t.title}
    end

    should 'return tabs with expected paths' do
      assert_equal ['path', 'path/tagging', 'path/metadata', 'path/history', 'path/admin'], tabs.map {|t| t.path('path')}
    end
